    ...
    for qa in qs_from:
        ...
        try:
            brand = BrandsDict.objects.filter(
                brand_supplier__ang_brand__icontains=qa[5].strip()
            ).distinct()
        except:
            brand = None
        if brand:
...
def test_category(request):  # Test function for mptt stuff
    return render(
        request, "product/test_genres.html", {"genres": Category.objects.all()}
    )
...
def show_category(request, hierarchy=None):
    ...
    try:
        ...
        return render(
            request, "product/category/categories.html", {"instance": instance}
        )
...
class Main(ListView):
    ...
    def get_queryset(self):
        ...
        if self.kwargs.get("pk", None):
            qs = (
                self.model.objects.filter(car_model=self.kwargs["pk"])
                .annotate(model_count=Count("car_model"))
                .order_by("name")
            )
            ...
            if letters:
                query = reduce(
                    lambda q, value: q | Q(name__istartswith=value), letters, Q()
                )
                qs = qs.filter(query).order_by("name")
        elif self.request.session["car"]["car_model_id"]:
            qs = self.model.objects.filter(
                car_model=self.request.session["car"]["car_model_id"]
            ).order_by("name")
        else:
...
class Detail(TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        context = {"object": product, "units_list": units_list}
        return render(request, self.template_name, context)
...
def main_work(request):
    ...
    nom_qs = Product.objects.filter(car_model=car, category__id=None).order_by("name")
    ...
    if request.GET.get("delete_group"):
        if request.GET.get("delete_group"):
            del_obj = Category.objects.get(pk=int(request.GET.get("delete_group")))
            del_obj.delete()
    ...
    if key_form.is_valid():
        ...
        nom_qs = (
            Product.objects.filter(
                Q(
                    reduce(operator.or_, (Q(name__icontains=x) for x in plus))
                    | Q(q_objects)
                )
            )
            .exclude(reduce(operator.or_, (Q(name__icontains=x) for x in minus)))
            .order_by("name")
        )
        ...
        for pl_and in plus_and:
            q_objects_key.add(
                Q(reduce(operator.and_, (Q(keywords__icontains=x) for x in pl_and))),
                Q.OR,
            )
    ...
    nom_count = Product.objects.filter(car_model=car, category__id=None).count()
    nom_count_tot = Product.objects.filter(car_model=car).count()
    counts = {"gk": group_count, "nk": nom_count, "nk_tot": nom_count_tot}
