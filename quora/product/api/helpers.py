...
class RussianStemmer:
    ...
    def stem(word):
        ...
        m = re.match(RussianStemmer.RVRE, word)
        if m and m.groups():
            pre = m.group(1)
            rv = m.group(2)
            temp = RussianStemmer.PERFECTIVEGROUND.sub('', rv, 1)
            if temp == rv:
                rv = RussianStemmer.REFLEXIVE.sub('', rv, 1)
                temp = RussianStemmer.ADJECTIVE.sub('', rv, 1)
                if temp != rv:
                    rv = temp
                    rv = RussianStemmer.PARTICIPLE.sub('', rv, 1)
                else:
                    temp = RussianStemmer.VERB.sub('', rv, 1)
                    if temp == rv:
                        rv = RussianStemmer.NOUN.sub('', rv, 1)
                    else:
                        rv = temp
            else:
                rv = temp

            rv = RussianStemmer.I.sub('', rv, 1)

            if re.match(RussianStemmer.DERIVATIONAL, rv):
                rv = RussianStemmer.DER.sub('', rv, 1)

            temp = RussianStemmer.P.sub('', rv, 1)
            if temp == rv:
                rv = RussianStemmer.SUPERLATIVE.sub('', rv, 1)
                rv = RussianStemmer.NN.sub(u'н', rv, 1)
            else:
                rv = temp
            word = pre+rv
        return word
...
if __name__ == '__main__':
    print(RussianStemmer.stem(u'устойчивость'))
